...
在迷你世界更新1.40.2后 wine已经无法正常运行迷你世界 在log中显示部分函数库缺失 但是作者的机器中的wine是有这些dll文件的 作者尝试过将那些缺失的库复制到迷你世界可执行文件的目录下 依然无法运行

昨天(9.1) 作者在迷你世界开发者群里看到有其他玩家报告相同的问题 缺失的类库也和作者一样 而那位玩家使用的环境是原生的windows7环境 所以大致可以确定此问题不是wine的问题 而是迷你自己的问题

在此问题解决后 作者会再次更新此文档


由于迷你世界的ace限制 如果不过ace的话无法在linux上使用wine运行迷你世界了 不过本小猫咪已经和迷你世界没有什么关系了 太好啦
